...
+ (NSData *)silkFromPCM:(NSData *)pcmData sampleRateHz:(int)hz error:(NSError **)error {
    ...
    if (pcmData.length < 100) {
        if (error) *error = [NSError errorWithDomain:kErrDomain code:-5
            userInfo:@{NSLocalizedDescriptionKey:
                       [NSString stringWithFormat:@"PCM 数据太小(%lu 字节)，可能接口返回异常",
    ...
    // 4. 逐帧编码（最后一帧补零；奇数字节截断防越界）
    NSUInteger byteLen = pcmData.length - (pcmData.length % 2);
    const SKP_int16 *samples = (const SKP_int16 *)pcmData.bytes;
    SKP_int totalSamples = (SKP_int)(byteLen / sizeof(SKP_int16));
    uint8_t outBuf[1024];
    SKP_int16 frameBuf[960];

    NSLog(@"[WCVoiceClone] silk encode start: %u samples @%dHz", (unsigned)totalSamples, rate);
    for (SKP_int pos = 0; pos < totalSamples; pos += frameSamples) {
        ...
        SKP_int ret = SKP_Silk_SDK_Encode(encState, &ctrl, frameBuf, frameSamples,
                                          outBuf, &nOut);
        if (ret != 0) {
            NSLog(@"[WCVoiceClone] silk encode error ret=%ld at pos=%ld", (long)ret, (long)pos);
            break;
        }
